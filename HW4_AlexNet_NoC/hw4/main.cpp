...
int sc_main(int argc, char *argv[])
{
    ...
    m_controller = new Controller(("m_controller").c_str(), tf);
    m_ROM        = new ROM(("m_ROM").c_str(), tf);
    ...
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 4; j++)
        {
            if (i == 0 && j == 0)
            {
                ...
                // To router0
                m_controller->flit_tx(c_r_flit[i][j][0]);
                m_controller->req_tx(c_r_req[i][j][0]);
                m_controller->ack_tx(c_r_ack[i][j][0]);

                // From router0
                m_controller->flit_rx(c_r_flit[i][j][1]);
                m_controller->req_rx(c_r_req[i][j][1]);
                m_controller->ack_rx(c_r_ack[i][j][1]);
