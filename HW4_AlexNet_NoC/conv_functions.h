...
// Change this function into an inline function


inline Tensor3d convolution(const Tensor3d& input, const Tensor4d& weights, const std::vector<float>& biases,
    ...
    Tensor3d padded_input(in_channel_num, std::vector<std::vector<float>>(in_height + 2 * padding, std::vector<float>(in_width + 2 * padding)));
    Tensor3d output(out_channel_num, std::vector<std::vector<float>>(out_height, std::vector<float>(out_width)));
...
inline Tensor3d reluLayer3d(const Tensor3d& input) {
...
inline Tensor1d reluLayer1d(const Tensor1d& input) {
...
inline Tensor3d maxpooling(const Tensor3d& input, int in_channel_num, int kernel_size, int stride) {
    ...
    Tensor3d output(in_channel_num, std::vector<std::vector<float>>(out_height, std::vector<float>(out_width)));
...
inline Tensor1d flattenTensor(const Tensor3d& tensor) {
...
inline Tensor1d fc_layer(const Tensor1d& input, const Tensor2d& weights, const Tensor1d& biases, int input_size, int output_size) {
...
inline Tensor1d softmax(const Tensor1d& input) {
...
inline std::vector<std::string> readClasses(const std::string& file_directory) {
...
inline std::string mapSoftmaxToClass(const Tensor1d& softmax, const std::vector<std::string>& classes) {
    float max_val = 0.0;
    int max_idx = 0;
...
inline void printSoftmaxValues(const Tensor1d& softmax, const std::vector<std::string>& classes) {
    for (size_t i = 0; i < softmax.size(); ++i) {
...
inline std::vector<std::pair<std::string, float>> sortClassesBasedOnSoftmax(const Tensor1d& softmax, const std::vector<std::string>& classes) {
    std::vector<std::pair<std::string, float>> class_softmax_pairs;

    for (size_t i = 0; i < softmax.size(); ++i) {
        class_softmax_pairs.push_back(std::make_pair(classes[i], softmax[i]));
    }

    std::sort(class_softmax_pairs.begin(), class_softmax_pairs.end(), [](const std::pair<std::string, float>& a, const std::pair<std::string, float>& b) {
