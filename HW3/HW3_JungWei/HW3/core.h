...
SC_MODULE(Core)
{
  ...
  PE pe;
  // The core id to specify which course is this
  int id;
  // The dump file pointer, for core dumping
  sc_trace_file *tf;

  // This is used as a packet pointer, to store the packet for easy management of packet sending and receiving
  Packet *p_tx, *p_rx;
  sc_lv<34> data_tx, data_rx;
  int data_tx_cnt;

  // Must use this constructor declaration to set the variable of sc_trace_file ptr
  SC_CTOR(Core);

  // The name of constructor correspondent to the declaration in main.cpp
  Core(sc_module_name name, int id, sc_trace_file *tf = nullptr) : sc_module(name), id(id)
  {
    ...
    // check if the trace file is not null, such that the trace file is instantiated correctly
    if (tf != nullptr)
    {
      // trace and dump the signals in a hierarchical way
      sc_trace(tf, clk, "core_" + std::to_string(id) + ".clk"); // the dump signal would be named as core_0.clk
  ...
  // Function tranforming from float to sc_lv<32>
  ...
  // Function transforming from sc_lv<32> to float
  ...
  //Function transforming from sc_lv<4> to int
  ...
  //main function, think of it as sequential logic which synchronizes with the clock
  void run()
  {
    if (rst.read() == 1)
    {
      // reseting the signals of ack, req, flit and the packet for core
      ack_rx.write(0);
      req_tx.write(0);
      flit_tx.write(0);
    ...
    {
      // start transmission, not initialized and sending done, the p_tx is set to nullptr
      if (p_tx == nullptr)
      {
        flit_tx.write(0);
        req_tx.write(0);
        p_tx = pe.get_packet();
        data_tx_cnt = 0; // Counting how many data has been sent right now
        if (p_tx != nullptr)
        {
      ...
      { // p_tx is not nullptr
        if (data_tx_cnt == 0) // transmitting the first data
        {
          data_tx[33] = 1; // BOP
          data_tx[32] = 0; // EOP
          ...
          // Note afterward unless dont transmitting, dont pull req_tx low
          req_tx.write(1);
          data_tx_cnt = data_tx_cnt + 1;
        }
        // perform two operations transmit and receive
        else if (ack_tx.read() == 1)
        {
          // transmist
          if (data_tx_cnt > p_tx->datas.size()) // count if done transmitting all the data
          {
            delete p_tx;
            p_tx = nullptr;
            req_tx.write(0);
          ...
          {
            // If this is the last data, send as tail
            data_tx[33] = 0; // BOP
            data_tx[32] = 1; // EOP
      ...
      if (ack_rx == 1)
      {
        // must keep oscilation between 0 and 1, to keep the data transmission
        ack_rx.write(0);
      }
      else if (req_rx.read() == 1)
      {
        // otherwise if there is a request to receive
        ...
        { // BOP
          ...
          {
            // create using packet struct, the packet receive
            p_rx = new Packet();
        ...
        {
          // EOP
          // The received flit is tail, stop receiving by invalidate the pointer
          if (p_rx == nullptr)
            cout << "Core_" << id << " ERROR: EOP received while not receiving packet" << endl;
          else
          {
            // Since datas is a vector, push back the data, and p_rx is a packet type
            // which are needed for check later on
        ...
        {
          // Simply push the needed data into the packet
          p_rx->datas.push_back(sclv32_2_float(data_rx.range(31, 0)));
          // std::cout << "Core_" << id << " receive " \
          //           << sclv32_2_float(data_rx.range(31, 0)) << std::endl;
        }
